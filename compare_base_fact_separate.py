#!/usr/bin/env python3
"""
BaseとFACTモデルの個別評価による比較スクリプト
メモリ干渉を避けるため、各モデルを個別に評価してから比較
"""
...
class BaseFactComparatorSeparate:
    def __init__(self, args):
        self.args = args
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        
    def evaluate_model_separately(self, model_type, checkpoint_dir, session):
        """指定されたモデルを個別に評価"""
        print(f"=== {model_type.upper()}モデル (Session {session}) を個別評価中 ===")
        
        # モデルのセットアップ
        if model_type == 'fact':
            model = FACT_MYNET(self.args, mode='ft_cos')
        else:
            model = BASE_MYNET(self.args, mode='ft_cos')
        ...
        print(f"{model_type.upper()}モデルを読み込みました: {checkpoint_path}")
    ...
    def analyze_results(self, fact_results, base_results, session):
        """結果を分析"""
        analysis = {
            'session': session,
            'test_class_count': fact_results['test_class'],
            'fact': {},
            'base': {},
            'comparison': {}
        }
        ...
        # 各モデルの分析
        for model_name, results in [('fact', fact_results), ('base', base_results)]:
    ...
    def evaluate_session(self, session, fact_checkpoint_dir, base_checkpoint_dir, output_dir):
        """指定されたセッションで両モデルを個別評価"""
        print("=" * 100)
        print(f"Session {session} の個別評価による比較を実行中...")
        
        # FACTモデルを個別評価
        fact_results = self.evaluate_model_separately('fact', fact_checkpoint_dir, session)
        if fact_results is None:
            return None
        
        # Baseモデルを個別評価
        base_results = self.evaluate_model_separately('base', base_checkpoint_dir, session)
        if base_results is None:
            return None
        
        # 結果を分析
        analysis_results = self.analyze_results(fact_results, base_results, session)
    ...
    def visualize_accuracy_comparison(self, analysis, session, save_dir):
        ...
        ax1.set_title(f'Overall Accuracy Comparison - Session {session} (Separate Evaluation)')
    ...
    def visualize_per_class_comparison(self, analysis, session, save_dir):
        ...
        ax1.set_title(f'Per-Class Accuracy Comparison - Session {session} (Separate Evaluation)')
    ...
    def save_comparison_statistics(self, analysis, session, save_dir):
        ...
        with open(os.path.join(save_dir, 'comparison_summary.txt'), 'w') as f:
            f.write(f"Base vs FACT Comparison - Session {session} (Separate Evaluation)\n")
            f.write("=" * 60 + "\n\n")
    ...
    def compare_all_sessions(self, all_results, output_dir):
        ...
        plt.title('Overall Accuracy Comparison Across Sessions (Separate Evaluation)')
        plt.xlabel('Session')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'overall_accuracy_comparison_separate.png'), 
                   dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.title('Base Classes Accuracy Comparison Across Sessions (Separate Evaluation)')
        plt.xlabel('Session')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'base_accuracy_comparison_separate.png'), 
                   dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.title('FACT Improvement Over Base Model Across Sessions (Separate Evaluation)')
        plt.xlabel('Session')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'improvement_comparison_separate.png'), 
                   dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def save_overall_comparison_summary(self, all_results, output_dir):
        ...
        with open(os.path.join(output_dir, 'overall_comparison_summary_separate.txt'), 'w') as f:
            f.write("Base vs FACT Overall Comparison Summary (Separate Evaluation)\n")
            f.write("=" * 70 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(description='BaseとFACTモデルの個別評価による比較')
    ...
    parser.add_argument('-output_dir', type=str, default='base_fact_comparison_separate',
                       help='出力ディレクトリ')
    ...
    # 比較器の初期化
    comparator = BaseFactComparatorSeparate(args)
    ...
    print(f"\n個別評価による比較分析完了！結果は {args.output_dir} に保存されました。")
