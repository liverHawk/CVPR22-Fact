    ...
    for session in range(args.start_session, args.sessions):
        ...
        if session == 0:
            ...
            if args.project == "fact":
                ...
                tsl, tsa = trainer.test_intergrate(
                    model, testloader, 0, args, session, validation=True
                )
            else:
                from models.base.helper import test
        ...
        print(
            f"session {session}: loss={tsl:.4f} acc={tsa:.4f}"
            if tsl is not None
            else f"session {session}: acc={tsa:.4f}"
        )
