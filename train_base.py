...
def get_command_line_parser():
    ...
    parser = argparse.ArgumentParser(
        description='FACT: Forward Compatible Few-Shot Class-Incremental Learning - Base Session Only',
    ...
    parser.add_argument('--epochs-new', type=int, 
                        ...
                        dest='epochs_new',
                        help='新規セッションのエポック数（このスクリプトでは使用されません）')
    ...
    parser.add_argument('--lr-new', type=float, 
                        ...
                        dest='lr_new',
                        help='新規セッションの学習率（このスクリプトでは使用されません）')
    ...
    parser.add_argument('--batch-size-new', type=int, 
                        ...
                        dest='batch_size_new',
                        help='新規セッションのバッチサイズ（このスクリプトでは使用されません）')
    ...
    parser.add_argument('--new-mode', type=str, 
                        ...
                        dest='new_mode',
                        help='新規セッションのモード（このスクリプトでは使用されません）')
    ...
    parser.add_argument('--model-dir', type=str, 
                        ...
                        dest='model_dir',
                        help='モデルパラメータの読み込み元ディレクトリ（既存モデルから開始する場合）')
...
if __name__ == '__main__':
    ...
    # base_sessionのみを実行するように設定
    args.start_session = 0
    # sessionsは後でset_up_datasetsで設定されるが、base_sessionのみなので1に設定
    # ただし、set_up_datasetsが呼ばれる前に設定する必要があるため、ここでは設定しない
    ...
    # base_sessionのみを実行するようにsessionsを1に設定
    # set_up_datasetsが呼ばれた後なので、args.sessionsは既に設定されている
    # ただし、base_sessionのみを実行するため、sessionsを1に上書き
    original_sessions = args.sessions
    args.sessions = 1
    
    print(f"Base session only mode: Running session 0 only (original sessions: {original_sessions})")
