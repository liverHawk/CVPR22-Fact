...
def get_command_line_parser():
    ...
    parser = argparse.ArgumentParser(
        description='FACT: Forward Compatible Few-Shot Class-Incremental Learning - New Sessions Only',
    ...
    parser.add_argument('--epochs-base', type=int, 
                        ...
                        dest='epochs_base',
                        help='ベースセッションのエポック数（このスクリプトでは使用されません）')
    ...
    parser.add_argument('--lr-base', type=float, 
                        ...
                        dest='lr_base',
                        help='ベースセッションの学習率（このスクリプトでは使用されません）')
    ...
    parser.add_argument('--batch-size-base', type=int, 
                        ...
                        help='ベースセッションのバッチサイズ（このスクリプトでは使用されません）')
    ...
    parser.add_argument('--base-mode', type=str, 
                        ...
                        dest='base_mode',
                        help='ベースセッションのモード（このスクリプトでは使用されません）')
...
if __name__ == '__main__':
    ...
    # --model-dirが必須であることを検証
    if args.model_dir is None or args.model_dir == '':
        print("Error: --model-dir is required for train_new.py")
        print("Please specify the path to the base session model (e.g., checkpoint/CICIDS2017_flow_improved/session0_max_acc.pth)")
        sys.exit(1)
    
    # モデルファイルの存在確認
    if not os.path.exists(args.model_dir):
        print(f"Error: Model file not found: {args.model_dir}")
        print("Please make sure the base session model exists. Run train_base.py first to create the base session model.")
        sys.exit(1)
    ...
    # new_sessionのみを実行するように設定
    args.start_session = 1
    # sessionsは後でset_up_datasetsで設定されるので、そのまま使用
    ...
    # trainerを初期化（set_up_datasetsが呼ばれる）
    trainer = importlib.import_module('models.%s.fscil_trainer' % (args.project)).FSCILTrainer(args)
    
    print(f"New sessions only mode: Running sessions 1 to {args.sessions - 1} (total sessions: {args.sessions})")
    print(f"Loading base session model from: {args.model_dir}")
