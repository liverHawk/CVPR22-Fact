...
class CICIDS2017_improved(Dataset):
    ...
    def __init__(self, root='data/', train=True,
        ...
        self.transform = None  # Add transform attribute for compatibility
                ...
                if isinstance(index, list) and len(index) > 0:
                    ...
                    if all(isinstance(i, (int, str)) for i in index):
                        ...
                        try:
                            ...
                            if all(0 <= idx < len(np.unique(self.targets)) for idx in class_indices):
                                self.data, self.targets = self.SelectfromClasses(self.data, self.targets, class_indices)
                            else:
                                # Otherwise treat as data indices
                                self.data, self.targets = self.SelectfromDataIndices(self.data, self.targets, class_indices)
                        except:
                            self.data, self.targets = self.SelectfromClasses(self.data, self.targets, index)
                    else:
                        self.data, self.targets = self.SelectfromClasses(self.data, self.targets, index)
    ...
    def SelectfromTxt(self, data2label, index_path):
        ...
        Remaps labels to be sequential [0, 1, 2, ...] based on unique classes.
        """
        ...
        unique_labels = np.unique(targets_tmp)
        label_map = {old_label: new_label for new_label, old_label in enumerate(unique_labels)}
        targets_tmp = np.array([label_map[label] for label in targets_tmp])

        # Update data2label mapping
        for i, target in enumerate(targets_tmp):
            data2label[i] = target
