...
class MiniImageNet(Dataset):
    def __init__(
        ...
        lines = [x.strip() for x in open(csv_path, "r")][1:]
    ...
    def SelectfromTxt(self, data2label, index_path):
        ...
        index = []
        lines = [x.strip() for x in open(index_path, "r")]
        for line in lines:
...
if __name__ == "__main__":
    ...
    trainset = MiniImageNet(
        root=dataroot, train=True, transform=None, index_path=txt_path
    )
