...
class MiniImageNet_concate(Dataset):
    def __init__(self, train,x1,y1,x2,y2):
        ...
        self.data=x1+x2
        self.targets=y1+y2
        logger.info("ImageNet1000_concate size: data=%d, targets=%d", len(self.data), len(self.targets))
...
if __name__ == '__main__':
    ...
    trainloader = torch.utils.data.DataLoader(dataset=trainset, batch_size=batch_size_base, shuffle=True, num_workers=8,
                                              pin_memory=True)
    logger.info("ImageNet1000 trainloader dataset size: %d", len(trainloader.dataset.data))