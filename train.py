...
def get_command_line_parser():
    ...
    parser.add_argument(
        "-project", type=str, default=yaml_defaults.get("project", PROJECT)
    )
    parser.add_argument(
        "-dataset",
        type=str,
        default=yaml_defaults.get("dataset", "CICIDS2017_improved"),
        choices=["mini_imagenet", "cub200", "cifar100", "CICIDS2017_improved"],
    )
    parser.add_argument(
        "-dataroot", type=str, default=yaml_defaults.get("dataroot", DATA_DIR)
    )
    parser.add_argument(
        "-encoder",
        type=str,
        default=yaml_defaults.get("encoder", "mlp"),
        choices=["mlp", "cnn1d"],
    )

    # about pre-training
    parser.add_argument(
        "-epochs_base", type=int, default=yaml_defaults.get("epochs_base", 400)
    )
    parser.add_argument(
        "-epochs_new", type=int, default=yaml_defaults.get("epochs_new", 100)
    )
    parser.add_argument(
        "-lr_base", type=float, default=yaml_defaults.get("lr_base", 0.005)
    )
    parser.add_argument("-lr_new", type=float, default=yaml_defaults.get("lr_new", 0.1))
    parser.add_argument(
        "-schedule",
        type=str,
        default=yaml_defaults.get("schedule", "Milestone"),
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        default=yaml_defaults.get("milestones", [50, 100, 150, 200, 250, 300]),
    )
    parser.add_argument("-step", type=int, default=yaml_defaults.get("step", 20))
    parser.add_argument(
        "-decay", type=float, default=yaml_defaults.get("decay", 0.0005)
    )
    parser.add_argument(
        "-momentum", type=float, default=yaml_defaults.get("momentum", 0.9)
    )
    parser.add_argument("-gamma", type=float, default=yaml_defaults.get("gamma", 0.25))
    parser.add_argument(
        "-temperature", type=float, default=yaml_defaults.get("temperature", 16)
    )
    parser.add_argument(
        "-not_data_init",
        action="store_true",
        help="using average data embedding to init or not",
    )
    parser.add_argument(
        "-batch_size_base", type=int, default=yaml_defaults.get("batch_size_base", 256)
    )
    parser.add_argument(
        "-batch_size_new",
        type=int,
        default=yaml_defaults.get("batch_size_new", 0),
        help="set 0 will use all the availiable training image for new",
    )
    parser.add_argument(
        "-test_batch_size", type=int, default=yaml_defaults.get("test_batch_size", 100)
    )
    parser.add_argument(
        "-base_mode",
        type=str,
        default=yaml_defaults.get("base_mode", "ft_cos"),
        choices=["ft_dot", "ft_cos"],
    )  # ft_dot means using linear classifier, ft_cos means using cosine classifier
    parser.add_argument(
        "-new_mode",
        type=str,
        default=yaml_defaults.get("new_mode", "avg_cos"),
    ...
    # for fact
    parser.add_argument(
        "-balance", type=float, default=yaml_defaults.get("balance", 0.01)
    )
    parser.add_argument(
        "-loss_iter", type=int, default=yaml_defaults.get("loss_iter", 0)
    )
    ...
    parser.add_argument(
        "-model_dir",
        type=str,
        default=yaml_defaults.get("model_dir", MODEL_DIR),
        help="loading model parameter from a specific dir",
    )
    parser.add_argument(
        "-set_no_val",
        action="store_true",
        help="set validation using test set or no validation",
    )

    # about training
    parser.add_argument("-gpu", default=yaml_defaults.get("gpu", "0"))
    parser.add_argument(
        "-num_workers", type=int, default=yaml_defaults.get("num_workers", 8)
    )
    ...
    # wandb logging
    parser.add_argument(
        "--use_wandb", action="store_true", help="Weights & Biasesへのログ送信を有効化"
    )
    parser.add_argument(
        "--wandb_project",
        type=str,
        default=yaml_defaults.get("wandb_project", None),
        help="wandbのプロジェクト名",
    )
    parser.add_argument(
        "--wandb_entity",
        type=str,
        default=yaml_defaults.get("wandb_entity", None),
        help="wandbのエンティティ（ユーザーまたはチーム）",
    )
    parser.add_argument(
        "--wandb_group",
        type=str,
        default=yaml_defaults.get("wandb_group", None),
        help="wandbのグループ名",
    )
    parser.add_argument(
        "--wandb_run_name",
        type=str,
        default=yaml_defaults.get("wandb_run_name", None),
        help="wandb上の実行名",
    )
    parser.add_argument(
        "--wandb_tags",
        nargs="*",
        default=yaml_defaults.get("wandb_tags", None),
        help="wandbタグ（スペース区切り）",
    )
    parser.add_argument(
        "--wandb_mode",
        type=str,
        default=yaml_defaults.get("wandb_mode", "online"),
    ...
    parser.add_argument(
        "--wandb_watch",
        type=str,
        default=yaml_defaults.get("wandb_watch", "gradients"),
