#!/usr/bin/env python3
"""
BaseとFACTモデルの同時比較スクリプト
両方のモデルを同じデータで評価し、詳細な比較分析を実行
"""
...
class BaseFactComparator:
    ...
    def load_model_checkpoints(self, fact_checkpoint_dir, base_checkpoint_dir):
        """両モデルのチェックポイントを読み込み"""
        print("チェックポイントを読み込み中...")
        
        # FACTモデルの読み込み
        fact_checkpoint_path = os.path.join(fact_checkpoint_dir, 'session0_max_acc.pth')
        ...
        # Baseモデルの読み込み
        base_checkpoint_path = os.path.join(base_checkpoint_dir, 'session0_max_acc.pth')
    ...
    def evaluate_session(self, session, output_dir):
        """指定されたセッションで両モデルを評価"""
        print("=" * 100)
    ...
    def save_comparison_statistics(self, analysis, session, save_dir):
        ...
        with open(os.path.join(save_dir, 'comparison_summary.txt'), 'w') as f:
            f.write(f"Base vs FACT Comparison - Session {session}\n")
            f.write("=" * 50 + "\n\n")
    ...
    def compare_all_sessions(self, all_results, output_dir):
        ...
        plt.title('Overall Accuracy Comparison Across Sessions')
        plt.xlabel('Session')
        ...
        plt.title('Base Classes Accuracy Comparison Across Sessions')
        plt.xlabel('Session')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'base_accuracy_comparison.png'), 
                   dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.title('FACT Improvement Over Base Model Across Sessions')
        plt.xlabel('Session')
    ...
    def save_overall_comparison_summary(self, all_results, output_dir):
        ...
        with open(os.path.join(output_dir, 'overall_comparison_summary.txt'), 'w') as f:
            f.write("Base vs FACT Overall Comparison Summary\n")
            f.write("=" * 60 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(description='BaseとFACTモデルの同時比較')
    ...
    parser.add_argument('-output_dir', type=str, default='base_fact_comparison',
                       help='出力ディレクトリ')
    ...
    # 比較器の初期化
    comparator = BaseFactComparator(args)
    
    # チェックポイントの読み込み
    if not comparator.load_model_checkpoints(args.fact_checkpoint_dir, args.base_checkpoint_dir):
        print("エラー: チェックポイントの読み込みに失敗しました")
        return
    
    # 各セッションを比較
    all_results = {}
    for session in sessions_to_compare:
        results = comparator.evaluate_session(session, args.output_dir)
        if results:
    ...
    print(f"\n比較分析完了！結果は {args.output_dir} に保存されました。")
